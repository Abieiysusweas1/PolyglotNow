<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['idioma'])) {
        ...
        if ($row[$nuevo_idioma] == 1) {
            $mensaje = "<p>Este idioma ya está activado.</p>";
        } elseif ($idiomas_activos < 5) {
            $update = "UPDATE usuarios SET $nuevo_idioma = 1 WHERE usuario = '$user'";
            if (mysqli_query($conexion,$update)) {
                header("Location: cursos.php");
                exit;
            } else {
                $mensaje = "<p>Error al añadir idioma: " . mysqli_error($conexion) . "</p>";
            }
        } else {
            $mensaje = "<p>" . $traducciones[$idioma_usuario]['max_idiomas'] . "</p>";
        }
    }
